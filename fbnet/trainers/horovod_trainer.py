...
from fbnet.trainers.base_trainer import BaseTrainer, sample_to_cuda
from fbnet.utils.config import prep_logger_and_checkpoint
from fbnet.utils.logging import print_config
from fbnet.utils.logging import AvgMeter
...
class HorovodTrainer(BaseTrainer):
    ...
    def train(self, dataloader, module, optimizer):
        ...
        f = open("log.txt",'a')
        f.write( 'Epoch {} | Avg.Loss {:.4f}'.format(module.current_epoch, self.avg_loss(output['loss'].item())))
        f.write('\n')
        f.close()
