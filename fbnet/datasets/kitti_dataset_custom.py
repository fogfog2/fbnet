...
from fbnet.datasets.kitti_dataset_utils import \
    pose_from_oxts_packet, read_calib_file, transform_from_rot_trans
from fbnet.utils.image import load_image
from fbnet.geometry.pose_utils import invert_pose_numpy
...
def read_png_depth(file):
  ...
  #  assert (np.max(depth_png) > 255), 'Wrong .png depth file'
    ...
    depth[depth_png == 0] = -1.
    return np.expand_dims(depth_png, axis=2)
...
########################################################################################################################
    ...
    def _get_intrinsics2(image_file, calib_data):
        """Get intrinsics from the calib_data dictionary."""
        #for cam in ['left', 'right']:
            # Check for both cameras, if found replace and return intrinsics
        #    if IMAGE_FOLDER[cam] in image_file:
...
########################################################################################################################
    ...
    def _get_depth_file_custom(self, image_file):
...
########################################################################################################################
    ...
    def __getitem__(self, idx):
        ...
        t2=c_data['P_rect_00']
        tt = np.reshape(c_data['P_rect_00'], (3, 4))[:, :3]
        sample.update({
            'intrinsics': tt,
        })
        ...
        #if self.with_depth:
        sample.update({
            'depth': self._get_depth_file_custom(self.paths[idx]),
        })
