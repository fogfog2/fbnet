...
from fbnet.utils.image import flip_model, interpolate_scales
from fbnet.geometry.pose import Pose
from fbnet.utils.misc import make_list


class SfmModel_depth_wofeedback(nn.Module):
    """
    ...
    kwargs : dict
        Extra parameters
    """
    def __init__(self, depth_net=None,depth_net2=None,  pose_net=None,
                 rotation_mode='euler', flip_lr_prob=0.0,
                 upsample_depth_maps=False, **kwargs):
        super().__init__()
        self.depth_net = depth_net
        self.depth_net2 = depth_net2
        ...
        self._network_requirements = {
                ...
                'depth_net2': True,  # Depth network required
    ...
    def add_depth_net2(self, depth_net2):
        """Add a depth network to the model"""
        self.depth_net2 = depth_net2
    ...
    def compute_inv_depths2(self, image, disp, random_seed):
        """Computes inverse depth maps from single images"""
    ...
    def forward(self, batch, return_logs=False):
        ...
        prev_inv_depths = self.compute_inv_depths(batch['rgb_context'][0], random_seed)
        inv_depths = self.compute_inv_depths2(batch['rgb'], random_seed)
        ...
        return {            
            ...
            'poses': pose,
            'prev_inv_depths' : prev_inv_depths,
        }
