...
from fbnet.datasets.transforms import get_transforms
from fbnet.utils.depth import inv2depth, post_process_inv_depth, compute_depth_metrics
from fbnet.utils.horovod import print0, world_size, rank, on_rank_0
from fbnet.utils.image import flip_lr
from fbnet.utils.load import load_class, load_class_args_create, \
    load_network, filter_args
from fbnet.utils.logging import pcolor
from fbnet.utils.reduce import all_reduce_metrics, reduce_dict, \
    create_dict, average_loss_and_metrics
from fbnet.utils.save import save_depth
from fbnet.models.model_utils import stack_batch
...
class ModelWrapper(torch.nn.Module):
    ...
    def configure_optimizers(self):
        ...
        # Depth optimizer
        if self.depth_net2 is not None:
            params.append({
                'name': 'Depth2',
                'params': self.depth_net2.parameters(),
                **filter_args(optimizer, self.config.model.optimizer.depth2)
            })
    ...
    def depth2(self, *args, **kwargs):
        """Runs the pose network and returns the output."""
        assert self.depth_net2 is not None, 'Depth network not defined'
        return self.depth_net2(*args, **kwargs)
        ...
        for n, metrics in enumerate(metrics_data):
            ...
            for key, metric in metrics.items():
                if self.metrics_name in key:                   
                    print(wrap(pcolor(num_line.format(*((key.upper(),) + tuple(metric.tolist()))), 'cyan')))
                    text = num_line.format(*((key.upper(),) + tuple(metric.tolist())))
                    f.write(text+'\n')
        f.write('\n')
        f.close()
...
def setup_depth_net2(config, prepared, **kwargs):
    """
    Create a depth network
    ...
    depth_net : nn.Module
        Create depth network
    """
    print0(pcolor('DepthNet2: %s' % config.name, 'yellow'))
    depth_net = load_class_args_create(config.name,
        paths=['fbnet.networks.depth2',],
        args={**config, **kwargs},
    )
    if not prepared and config.checkpoint_path is not '':
        depth_net = load_network(depth_net, config.checkpoint_path,
                                 ['depth_net2', 'disp_network2'])
    return depth_net
...
def setup_model(config, prepared, **kwargs):
    ...
    model = load_class(config.name, paths=['fbnet.models',])(
        **{**config.loss, **kwargs})
    ...
    # Add depth network if required
    if model.network_requirements['depth_net2']:
        model.add_depth_net2(setup_depth_net2(config.depth_net2, prepared))
...
def setup_dataset(config, mode, requirements, **kwargs):
    ...
    for i in range(len(config.split)):
        ...
        if config.dataset[i] == 'KITTI':
            from fbnet.datasets.kitti_dataset import KITTIDataset
        ...
        # DGP dataset
            ...
            from fbnet.datasets.dgp_dataset import DGPDataset
            dataset = DGPDataset(
        ...
        # Image dataset
            ...
            from fbnet.datasets.image_dataset import ImageDataset
            dataset = ImageDataset(
                config.path[i], config.split[i],
                **dataset_args, **dataset_args_i,
            )
        elif config.dataset[i] == "Custom":
            from fbnet.datasets.kitti_dataset_custom import CustomDataset
